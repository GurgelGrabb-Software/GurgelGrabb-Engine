...
{
    int w, h, ch;
    const PixelData data = (PixelData)stbi_load(filepath.c_str(), &w, &h, &ch, 0);
...
void CImage::SaveToFile(CImage& image, const std::filesystem::path& filepath)
{
    ...
    stbi_write_png(filepath.c_str(), image._width, image._height, channels, image._pixels, image._width * channels);
}
